...
function Employees() {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get('/api/faculty'); // keeping the endpoint as faculty for now
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingEmployee) {
        await axios.put(`/api/faculty/${editingEmployee.EmployeeID}`, formData);
      } else {
      ...
      try {
        ...
        if (error.response?.data?.message?.includes('teaching')) {
          if (window.confirm(error.response.data.message + '\nClick OK to delete everything.')) {
            ...
            const forceResponse = await axios.delete(`/api/faculty/${id}?force=true`);
