    ...
    try {
      ...
      const departmentsWithCounts = await Promise.all(
        response.data.map(async (dept) => {
          const detailsResponse = await axios.get(`http://localhost:5000/api/departments/${dept.DepartmentID}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingDepartment) {
        await axios.put(`http://localhost:5000/api/departments/${editingDepartment.DepartmentID}`, formData);
      } else {
