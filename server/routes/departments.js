...
router.delete('/:id', async (req, res) => {
  try {
    ...
    try {
      ...
      const { StudentCount, EmployeeCount, CourseCount } = checkResult.recordset[0];
      
      if (StudentCount > 0 || EmployeeCount > 0 || CourseCount > 0) {
        const dependencies = [];
        if (StudentCount > 0) dependencies.push(`${StudentCount} students`);
        if (EmployeeCount > 0) dependencies.push(`${EmployeeCount} employees`);
